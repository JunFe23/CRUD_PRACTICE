...
<c:choose>
   <c:when test="${id eq '' or id eq null}">
     <c:redirect url="/bootBoard/signin.jsp"></c:redirect>
   </c:when>
</c:choose>
<head>
	   ...
	   $("i#ok").click(function(){
		   //alert($("textarea").val().length);
			   if($("input[name='password']").val().length==0||
					  ...
					  $("input[name='writer']").val().length==0){
				  alert("Check");
				$("input").each(function(){				
					$(this).val('');
				});
...
<body>
	...
	<form action="/web/mvcBoardInsert.do" name="frm" method="POST" enctype="multipart/form-data">
	<div id="write-field">
		...
		<div class="col-md-6">
						...
						<div class="row row-xs-bskr">
							<div class="col-md-6 col-sm-6 col-xs-bskr">
								...
								<div class="input-forms">
									<input size="20" type="text" name="writer" 
										class="form-control input-sm input-name bskr-font user" />
									<input size="20" type="hidden" name="id" value="${id}"
										class="form-control input-sm input-name bskr-font user" />	
								</div>
			...
			<div class="bottombox">
				<input type="reset" id="cancel" value="취소" class="btn btn-default btn-sm"/>&nbsp;
				<button id="submitPost" type="button" class="btn btn-primary btn-sm">
					<i class="glyphicon glyphicon-ok" id="ok">확인</i> 
				</button>
			</div>
		</div>
	</div>
